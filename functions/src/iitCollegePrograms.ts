...
const COLLEGE_CODE_SET = new Set<string>(COLLEGE_CODES);
...
export function isCollegeCode(code: string | undefined): code is CollegeCode {
  const c = code?.trim();
...
export function isProgramInCollege(
  ...
  if (!p || !isCollegeCode(collegeCode)) return false;
  return IIT_COLLEGE_PROGRAMS[collegeCode].includes(p);
}
