...
const TREND_GAP_NOTE =
  "Only days you checked in are shown. Gaps mean no log was recorded.";
...
export function LineTrendChart({
  ...
  const innerW = fitToWidth
    ? Math.max(1, fitW - PAD_BASE.l - PAD_BASE.r)
    : minInnerW;
  ...
  useEffect(() => {
    ...
    const targetX = Math.max(
      0,
      xAt(latestLoggedIndex) - PAD_BASE.l - fitW * 0.45,
    );
    const id = setTimeout(() => {
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      <Text style={{ color: AURORA.textMuted, fontSize: 11, marginTop: 8 }}>
        {TREND_LEGEND}
      </Text>
      <Text
        style={{
...
export function TaskLoadBarChart({
  title,
  caption,
  tasks,
  labels,
  ...
  if (total === 0) {
    return (
      <View style={{ marginBottom: 20 }}>
        ...
        >
          <Text
            style={{ color: AURORA.textSec, fontSize: 14, textAlign: "center" }}
          >
            No check-ins in this two-week window yet. After you log, you will
            see how your days split out.
          </Text>
        </View>
      </View>
    );
...
export function MoodWeekStrip({ points }: { points: DailyChartPoint[] }) {
  ...
  return (
    <View>
      ...
      >
        One square per day (left = older, right = today). Color matches your
        mood scale (1–5). Swipe sideways on small screens.
      </Text>
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
      ...
      >
        {points.map((p, i) => {
          ...
          if (reduceMotion) {
            return <View key={p.dateKey}>{Cell}</View>;
          }
          return (
            <Animatable.View
              key={p.dateKey}
