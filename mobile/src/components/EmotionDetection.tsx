...
export function EmotionDetection({
  ...
  const getEmotionAnalysis = async (
    ...
    try {
      console.log(
        "Sending request to:",
        `${API_URL}/api/emotion/analyze-upload`,
      );
      ...
      if (result.emotions) {
        predictions = Object.entries(result.emotions).map(
          ([emotion, score]) => ({
          ...
          >
            <Image
              ...
              style={{
                width: "100%",
                height: "100%",
                transform: capturedWithFrontCamera
                  ? [{ scaleX: -1 }]
                  : undefined,
              }}
              resizeMode="cover"
          ...
          {detectedEmotions.length > 0 && (
            <View style={{ width: "100%", marginBottom: 16 }}>
              ...
              {detectedEmotions.map((emotion, index) => {
                const emotionColor =
                  emotion.color || getEmotionColor(emotion.emotion);
      ...
      <Modal visible={isCameraVisible} animationType="slide">
        <View style={{ flex: 1, backgroundColor: "black" }}>
          {permission?.granted ? (
            ...
            >
              <Text style={{ color: AURORA.textPrimary, marginBottom: 16 }}>
