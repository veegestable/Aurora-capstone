...
export function polishStudentBullet(text: string): string {
  ...
  const taskMatch = t.match(
    /^total logged tasks across the week:\s*(\d+)\.?$/i,
  );
  if (taskMatch) {
...
export function dominantStressPlain(band: DayStress | "—"): string {
  if (band === "—" || band === "None")
    return "We need more check-ins to describe the week.";
  if (band === "Low")
    return "Most of your days showed a lower Stress Index this week.";
  if (band === "Moderate")
    return "Most of your days sat in the middle on the Stress Index.";
