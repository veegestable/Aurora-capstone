/**
 * Role-aware session status copy — single source for student & counselor UIs.
 */
...
export type SessionPresentationContext = {
  ...
  /** Student: counselor sent proposed slots on a pending session. */
...
export function sessionPresentationColors(variant: SessionPresentationVariant): {
  ...
  switch (variant) {
    case "green":
      return { bg: "rgba(34,197,94,0.2)", text: AURORA.green };
    case "blue":
      return { bg: "rgba(59,130,246,0.2)", text: AURORA.blueLight };
    case "orange":
      return { bg: "rgba(249,115,22,0.2)", text: AURORA.orange };
    case "red":
      return { bg: "rgba(239,68,68,0.2)", text: AURORA.red };
    case "muted":
      return { bg: "rgba(255,255,255,0.08)", text: AURORA.textMuted };
    case "amber":
    default:
      return { bg: "rgba(254,189,3,0.2)", text: AURORA.amber };
  }
}

/**
 * Human-readable status for chat cards, modals, and list chips.
 */
export function getSessionPresentation(
  ctx: SessionPresentationContext,
): SessionPresentation {
  ...
  if (st === "completed") {
    return {
      pillLabel: "Completed",
      subtitle: student
        ? "This counseling session is complete."
        : "Marked as completed.",
      variant: "blue",
    };
  }
  ...
  if (st === "rescheduled") {
    return {
      ...
      subtitle: "A new time is being arranged.",
      variant: "amber",
    };
  }
  ...
  if (st === "declined" || st === "approved") {
    return {
      pillLabel: st === "declined" ? "Declined" : "Scheduled",
      subtitle:
        st === "declined"
          ? "This request was declined."
          : "Session approved.",
      variant: st === "declined" ? "red" : "green",
    };
  }
...
/** Counselor chat pills historically use uppercase labels. */
export function formatCounselorSessionPillLabel(label: string): string {
  return label.toUpperCase();
}

/** Student Mood Log / My sessions chip (short). */
export function getStudentSessionChipLabel(
  status: string,
  opts?: { counselorOfferedSlots?: boolean },
): string {
  return getSessionPresentation({
    status,
    role: "student",
    counselorOfferedSlots: opts?.counselorOfferedSlots,
  }).pillLabel;
}

/**
 * Counselor Session History — derived list/detail badge from {@link SessionHistoryBadge}
 * (timeline: upcoming vs later today vs outcomes vs reschedule window).
 */
