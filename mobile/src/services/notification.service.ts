...
export const notificationService = {
  async getNotifications(userId: string): Promise<Notification[]> {
    try {
      ...
      return notifications.map((n) => {
        ...
        const scheduledFor =
          n.scheduled_for instanceof Timestamp
            ? n.scheduled_for.toDate()
            : new Date(n.scheduled_for);
        const createdAt =
          n.created_at instanceof Timestamp
            ? n.created_at.toDate()
  ...
  async createNotification(data: {
    ...
    try {
      ...
      const scheduledFor =
        result.scheduled_for instanceof Timestamp
          ? result.scheduled_for.toDate()
          : new Date(result.scheduled_for);
      const createdAt =
        result.created_at instanceof Timestamp
          ? result.created_at.toDate()
