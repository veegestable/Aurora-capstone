...
import SessionHistoryDetailModal, {
...
export default function SessionHistoryScreen() {
  ...
  const groupedByDate = useMemo(() => {
    ...
    for (const s of filteredSessions) {
      ...
      const key =
        d && !isNaN(d.getTime()) ? d.toDateString() : `fallback_${s.id}`;
    ...
    for (const k of Object.keys(groups)) {
      groups[k].sort((a, b) => {
        const da =
          getSessionDate(a)?.getTime() ??
          (a.updatedAt instanceof Date
            ? a.updatedAt.getTime()
            : new Date(a.updatedAt).getTime());
        const db =
          getSessionDate(b)?.getTime() ??
      ...
      setSessions((prev) =>
        prev.map((s) => {
          ...
          return {
            ...s,
            status: nextStatus,
            sessionHistoryBadge: computeSessionHistoryBadge({
              ...s,
              status: nextStatus,
            }),
          };
            ...
            <>
              ...
              >
                ...
                <Text
                  style={{
                    color:
                      badge === "expired" ? AURORA.textMuted : AURORA.orange,
                    fontSize: 13,
                    fontWeight: "600",
                    flex: 1,
                  }}
