...
function latestLogForDay(logs: MoodLogEntryRow[], dayKey: string): MoodLogEntryRow | null {
  const dayLogs = logs.filter((l) => {
    const ld = l.timestamp instanceof Date ? l.timestamp : new Date(l.timestamp)
...
export function buildLast7DaysPayload(logs: MoodLogEntryRow[]): WeeklySeriesPayload {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const mood = energyLevelToMoodScale(log.energy ?? 5)
...
export function summarizeWeekSeries(payload: WeeklySeriesPayload): {
  ...
  if (bands.length) {
    ...
    dominantStress =
      (Object.entries(counts).sort((a, b) => b[1] - a[1])[0]?.[0] as StressBand) ?? '—'
  }
